...
from .TSA import TwoStageAttentionLayerCrossSegmentsMultiMTS, TwoStageAttentionLayerCrossSegments
...
class scale_block(nn.Module):
    ...
    '''
    def __init__(self, win_size, d_model, n_heads,sparsity, d_ff, depth, dropout, \
        ...
        for i in range(depth):
            self.encode_layers.append(TwoStageAttentionLayerCrossSegmentsMultiMTS(seg_num, factor, d_model, n_heads,sparsity, \
                                                        d_ff, dropout))
...
class Encoder(nn.Module):
    ...
    def __init__(self, e_blocks, win_size, d_model, n_heads, d_ff, block_depth, dropout,
        ...
        self.sparsity_list = [104,104]
        self.encode_blocks.append(scale_block(1, d_model, n_heads,self.sparsity_list[0], d_ff, block_depth, dropout,\
                                            in_seg_num, factor))
        for i in range(1, e_blocks):
            self.encode_blocks.append(scale_block(win_size, d_model, n_heads,self.sparsity_list[1], d_ff, block_depth, dropout,\
